...
async function handleLocalDownloadMulti() {
  ...
  try {
    ...
    // Safari drops part of a burst of automatic downloads. Wait for each
    // download to finish before starting the next one.
    for (const torrent of torrents) {
      try {
        const result = await sendMessage("downloadTorrent", { torrent, downloaderId: "local" });
        if (result.downloadStatus === "failed") failedCount += 1;
      } catch {
        failedCount += 1;
      }
    }
    ...
    if (failedCount > 0) {
      runtimeStore.showSnakebar(`${failedCount}/${torrents.length} torrent downloads failed`, { color: "error" });
    }
  } finally {
