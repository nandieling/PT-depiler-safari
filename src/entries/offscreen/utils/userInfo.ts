...
flushQueue.on("active", async () => {
  ...
  // Safari user-info requests are relayed through content scripts. Existing
  // installations may still have the upstream default of 5 persisted, which
  // makes the cookie lookup and relay decision race across sites.
  const queueConcurrency = __BROWSER__ === "safari" ? 1 : (configStoreRaw?.userInfo?.queueConcurrency ?? 1);
...
export async function getSiteUserInfoResult(siteId: string) {
  try {
    return (await flushQueue.add(async () => {
      ...
      // Safari does not expose these site cookies, and its cookies API can
      // leave this optional maintenance request pending indefinitely.
    ...
    const failedUserInfo: IUserInfo = {
    ...
    logger({
      msg: `getSiteUserInfoResult for ${siteId} failed: ${errorMessage}`,
      data: failedUserInfo,
      level: "error",
    });
    await setSiteLastUserInfo(failedUserInfo).catch(() => undefined);
