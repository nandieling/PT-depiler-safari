...
export async function getSiteUserConfig(siteId: TSiteID, flush = false) {
  ...
  // Older editor builds could retain a shared object and write a site's last
  // user-info object into another site's nested configuration.
  let sanitizedStoredConfig = false;
  for (const field of ["merge", "inputSetting"] as const) {
    const nestedConfig = storedSiteUserConfig[field] as Record<string, unknown> | undefined;
    ...
    for (const [key, value] of Object.entries(nestedConfig)) {
      if (
        ...
        typeof value === "object" &&
        "site" in value &&
        (value as { site?: unknown }).site === key &&
        "status" in value &&
        "updateAt" in value
      ) {
        delete nestedConfig[key];
        sanitizedStoredConfig = true;
      }
    }
  }
  if (sanitizedStoredConfig) {
