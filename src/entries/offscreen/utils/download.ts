...
async function downloadTorrentToLocalFile(
  ...
  if (
    ...
    __BROWSER__ !== "safari" &&
    ["GET", "POST"].includes(downloadMethod.toUpperCase())
  ) {
    try {
  ...
  if (localDownloadMethod === "extension") {
    try {
      ...
      const torrentBlob = torrentInstance.metadata.blob();
      ...
      if (__BROWSER__ === "safari") {
        saveAs(torrentBlob, filename);
      } else {
        const torrentUrl = URL.createObjectURL(torrentBlob);
