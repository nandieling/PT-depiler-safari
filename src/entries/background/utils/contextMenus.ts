...
function showDownloadNotification(message: string) {
...
async function downloadLinkPush(
  ...
  sendMessage("downloadTorrent", {
    ...
    .then((result) => {
      ...
      showDownloadNotification(chrome.i18n.getMessage(messageName, [downloader.name]));
    })
    .catch(() => {
      showDownloadNotification(chrome.i18n.getMessage("notificationSendLinkToDownloaderFailure", [downloader.name]));
    });
}
