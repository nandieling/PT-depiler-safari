...
// 存储恢复是异步的；Safari 上页面启动较快时，直接读取会得到空站点列表。
onMounted(async () => {
  await metadataStore.$onReady();
  await initTableData();
});

// 添加或移除站点后立即同步“我的数据”，无需重开页面。
watch(
...
async function multiOpen() {
  const selectedSites = [...tableSelected.value];
  const siteUrlResults = await Promise.allSettled(
    selectedSites.map(async (siteId) => ({ siteId, url: await metadataStore.getSiteUrl(siteId) })),
  );
  const siteUrls = siteUrlResults.flatMap((result) => (result.status === "fulfilled" ? [result.value] : []));

  let openedSiteCount = 0;
  for (const { url } of siteUrls) {
    if (!url) continue;
    try {
      // Load only the first site. Safari keeps never-activated background tabs
      // suspended until the user selects them, avoiding a burst of site loads.
      await chrome.tabs.create({ url, active: openedSiteCount === 0 });
      openedSiteCount += 1;
    } catch (error) {
      console.warn(`[PTD] Failed to open site tab: ${url}`, error);
    }
  }
}

async function openSiteInNewTab(siteId: TSiteID) {
          ...
          >
            <div class="favicon-hover-wrapper favicon-hover-bg">
              <SiteFavicon
                ...
                @click="() => openSiteInNewTab(item.site)"
              />
            </div>
          </v-badge>
