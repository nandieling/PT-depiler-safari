...
export interface ISafariTabRequest {
  url: string;
  method: "GET" | "HEAD";
...
export interface ISafariTabResponse {
  body: string;
  finalUrl: string;
  headers: Record<string, string>;
  status: number;
...
interface ProtocolMap extends TMessageMap {
  ...
  safariTabRequest(data: ISafariTabRequest): ISafariTabResponse;
...
function createMessageWrapper<PM extends ProtocolMap>(original: {
  ...
  const wrappedSendMessage = async <K extends keyof PM>(
    ...
    if (__BROWSER__ != "chrome" && typeof data !== "undefined") {
      data = JSON.parse(JSON.stringify(data)); // 统一 background page 下深拷贝数据，避免传递 proxy 出错
    }
...
export const { sendMessage, onMessage } = createMessageWrapper(
  defineExtensionMessaging<ProtocolMap>({
    logger: __BROWSER__ != "chrome" || isDebug ? console : undefined,
  }),
);
