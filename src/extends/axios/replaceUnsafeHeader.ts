  ...
  axiosAllowUnsafeHeaderInstance.interceptors.request.use(async function (config) {
    if (config.headers) {
      ...
      if (requestHeaders.length > 0) {
        ...
        // Safari can classify cross-origin requests from an extension page as
        // "other", so limiting the rule to XMLHttpRequest prevents Cookie
        // injection even though Axios itself is using XHR.
        if (__BROWSER__ !== "safari") {
          rule.condition.resourceTypes = ["xmlhttprequest" as chrome.declarativeNetRequest.ResourceType.XMLHTTPREQUEST];
        }

        // Safari extension-page requests can carry a real tab id. Do not
        // exclude tabs there, or the Cookie replacement rule never matches.
        await sendMessage("updateDNRSessionRules", { rule, extOnly: __BROWSER__ !== "safari" });
      }
    }
