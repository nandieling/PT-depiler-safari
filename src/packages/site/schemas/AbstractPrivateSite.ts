...
export default class PrivateSite extends BittorrentSite {
  ...
  protected override async executeRequest<T>(axiosConfig: AxiosRequestConfig): Promise<AxiosResponse<T>> {
    axiosConfig.withCredentials ??= true;

    if (__BROWSER__ === "safari" && [undefined, "document", "json", "text"].includes(axiosConfig.responseType)) {
      const { applySafariRequestCookies, executeSafariTabRequest, shouldUseSafariTabRequest } =
        await import("../utils/safariRequest.ts");

      await applySafariRequestCookies(axiosConfig);
      if (shouldUseSafariTabRequest(axiosConfig)) {
        return await executeSafariTabRequest<T>(axiosConfig);
      }

      const method = (axiosConfig.method ?? "GET").toUpperCase();
      const canRelay = method === "GET" || method === "HEAD";
      let response: AxiosResponse<T>;
      try {
        response = await super.executeRequest<T>(axiosConfig);
      } catch (error) {
        if (canRelay) {
          return await executeSafariTabRequest<T>(axiosConfig);
        }
      ...
      if (canRelay && !this.loggedCheck(response)) {
        return await executeSafariTabRequest<T>(axiosConfig);
      }
      return response;
    }

    return await super.executeRequest<T>(axiosConfig);
  }
  ...
  public async getUserInfoResult(lastUserInfo: Partial<IUserInfo> = {}): Promise<IUserInfo> {
    ...
    try {
      ...
      console.error(`[Site] ${this.name} failed to refresh user information: ${errorMessage}`);

      flushUserInfo.status = EResultParseStatus.parseError;
      flushUserInfo.errorMessage = errorMessage;
