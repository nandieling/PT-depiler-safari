...
export default class Keepfrds extends NexusPHP {
  protected override async parseUserInfoForUploads(flushUserInfo: Partial<IUserInfo>): Promise<Partial<IUserInfo>> {
    try {
      return await super.parseUserInfoForUploads(flushUserInfo);
    } catch (error) {
      if (error instanceof Error && /Network Error:\s*404\b/.test(error.message)) {
        console.warn("[Site] KEEPFRDS does not expose the optional uploaded-torrents endpoint; skipping uploads.");
